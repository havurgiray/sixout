...
struct DigitalOutputContext
{
  ...
  DigitalOutputContext(AudioObjectID device, AudioObjectID stream, const AudioStreamBasicDescription &format,
    const AudioChannelLayoutTag channelLayoutTag, bool useDLPiiUpmix, int codec = 0 /* 0 = AC-3, 1 = DTS */, int bitRate = 0);

  const char *GetCodecName() const { return _encoder.GetCodecName(); }
  int GetBitRate() const { return _encoder.GetBitRate(); }

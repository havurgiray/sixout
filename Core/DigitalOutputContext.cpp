...
DigitalOutputContext::DigitalOutputContext(AudioObjectID device, AudioObjectID stream,
  const AudioStreamBasicDescription &format, const AudioChannelLayoutTag channelLayoutTag, bool useDLPiiUpmix, int codec, int bitRate)
: _device(device), _stream(stream), _format(format), _channelLayoutTag(channelLayoutTag)
, _log(os_log_create("local.sixout", "OutIOProc"))
, _encoder(GetBestInputFormatForOutputFormat(_format, _channelLayoutTag),
    _channelLayoutTag, _format, _log, useDLPiiUpmix, codec == 1 ? AV_CODEC_ID_DTS : AV_CODEC_ID_AC3, bitRate)
  ...
  const AudioTimeStamp* inOutputTime, void* inClientData)
{
  ...
  if (outOutputData->mBuffers[0].mDataByteSize < me->_format.mBytesPerPacket)
  { // this can happen during start-up / initialization, so just emit the (prefilled) zeros
    os_log_info(me->_log, "Output buffer size (%u) too small for packet size (%u)", outOutputData->mBuffers[0].mDataByteSize, me->_format.mBytesPerPacket);
    return noErr;
  }

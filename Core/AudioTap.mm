...
, _deviceUID([deviceUID copy])
{
  ...
  tapConfig.name = [NSString stringWithFormat:@"SixOut Tap for '%@'", deviceUID];
...
: _audioTap(std::move(audioTap))
{
  NSDictionary *dict = @{
    ...
    @kAudioAggregateDeviceNameKey : [NSString stringWithFormat:@"SixOut Aggregate for '%@'", _audioTap._deviceUID],
